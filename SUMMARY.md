# Table of contents

* [java笔记目录](README.md)
* [Java技术大纲](java-overview.md)

## 容器

* [1. sorted collection](collection/1.sorted-collections.md)
* [2. collections usages](collection/2.collections-usage.md)
* [3. Queue and Deque](collection/3.queue-and-deque.md)
* [4. Enumeration interface](collection/4.-enumeration-interface.md)
* [5. HashTable and Properties](collection/5.-hashtable-ji-properties-zi-lei.md)
* [6. Map and its implements](collection/6.-map.md)
* [7. Guava](collection/7.-guava.md)
* [8. Table](collection/8.-table.md)
* [9. Commons](collection/9.-commons.md)
* [10. Summary](collection/10.-zong-jie.md)
* [IO](collection/io.md)
* [1. input and output](collection/1.-wen-jian-io.md)
* [2. cache](collection/2.-huan-cun.md)

## thread

* [1. Foundation](thread/1.foundation.md)
* [2. concurrent](thread/2.-concurrent.md)
* [3. java.util.concurrent](thread/3.-java.util.concurrent.md)

## 网络编程

* [java.net](net-programing/java.net.md)
* [netty](net-programing/netty.md)

## java动态性

* [1. 注解与反射](java-dynamic/1.annotation-and-reflect.md)
* [2. 动态编译](java-dynamic/2.dynamic-compiling.md)
* [3. JVM核心机制与类加载器](java-dynamic/2.-jvm-and-classloader.md)
* [4. 内部类](java-dynamic/4.inner-classes.md)

## 设计模式

* [1. 单例模式](design-pattern/1.singleton.md)
* [2. 工厂模式](design-pattern/2.factory.md)
* [3. 代理模式](design-pattern/3.proxy.md)
* [其他模式](design-pattern/4.others.md)

## JVM

* [内存模型](jvm/memory-model.md)
* [synchronized锁机制](jvm/synchronized.md)

## 新特性

* [Stream in jdk8](new-features/stream-in-jdk8.md)

## 框架

* [MySql索引](frameworks/index-in-mysql.md)
* [跨数据库的分布式事务](frameworks/transaction-cross-database.md)
* [Spring功能扩展](frameworks/spring-kuo-zhan.md)
* [Spring](frameworks/spring-study-map.md)

